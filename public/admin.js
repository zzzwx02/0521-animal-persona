...
function getNickname(result) {
  return result.nickname || result.answers?._meta?.nickname || "-";
}

function getFullName(result) {
  return result.full_name || result.primary_animal_name || result.primary_animal || "-";
}

function getCode(result) {
  return result.code || result.secondary_animal || "-";
}
...
function renderSummary(summary) {
  ...
  renderBars(dimensionAverages, dimensionEntries, 100, (value) => `${value}`);
}

function renderAnswers(answers) {
  const entries = Object.entries(answers || {}).filter(([questionId]) => !questionId.startsWith("_"));
      ...
      <div class="answer-list">
        ${entries
          .map(([questionId, answer]) => {
            const score = answer.score ?? (Number(answer.index) >= 0 ? Number(answer.index) + 1 : "-");
            const label = answer.label || answer.choice || "-";
            const dimension = answer.dimension || answer.dim || "-";
            return `<span>${questionId.toUpperCase()} · ${score} · ${label} · ${DIMENSION_LABELS[dimension] || dimension}</span>`;
          })
          .join("")}
      </div>
    </details>
...
function renderScores(result) {
  const scores = result.scores || result.raw_scores || {};
  const preferredKeys = ["energy", "information", "decision", "lifestyle", "state"];
  const keys = preferredKeys.some((key) => scores[key] !== undefined)
    ? preferredKeys
    : ["agency", "novelty", "social", "method", "sensitivity"];

  return keys
    .filter((key) => scores[key] !== undefined)
    .map((key) => `${DIMENSION_LABELS[key] || key} ${scores[key]}`)
    .join(" · ");
}

function renderRows(results) {
  if (!results.length) {
    resultRows.innerHTML = `<tr><td colspan="10"><div class="empty-state">还没有收到测试结果。</div></td></tr>`;
    return;
  }
    ...
    .map(
      (result) => `
        <tr>
          ...
          <td><strong>${getNickname(result)}</strong></td>
          <td><strong>${getFullName(result)}</strong></td>
          <td class="mono">${getCode(result)}</td>
          <td>${result.state_name || result.state_type || "-"}</td>
          <td>${confidenceName(result.confidence)}</td>
          <td>${renderScores(result) || "-"}</td>
...
async function loadResults() {
  try {
    ...
    const message = error.message || "数据接口暂时不可用";
    animalDistribution.innerHTML = `<div class="empty-state">没有读取到线上数据，请检查 Vercel 环境变量和 Supabase 配置。</div>`;
    dimensionAverages.innerHTML = `<div class="empty-state">${message}</div>`;
    resultRows.innerHTML = `<tr><td colspan="10"><div class="empty-state">请确认新版项目已部署，并且 /api/results 可以访问。</div></td></tr>`;
  }
}
